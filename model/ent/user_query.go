...
// QueryArtilces chains the current query on the "artilces" edge.
func (uq *UserQuery) QueryArtilces() *ArticleQuery {
	...
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.O2M, false, user.ArtilcesTable, user.ArtilcesColumn),
		)
...
func (uq *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withArtilces: uq.withArtilces.Clone(),
...
// WithArtilces tells the query-builder to eager-load the nodes that are connected to
// the "artilces" edge. The optional arguments are used to configure the query builder of the edge.
func (uq *UserQuery) WithArtilces(opts ...func(*ArticleQuery)) *UserQuery {
	query := (&ArticleClient{config: uq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	uq.withArtilces = query
	return uq
}
...
func (uq *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := uq.withArtilces; query != nil {
		if err := uq.loadArtilces(ctx, query, nodes,
			func(n *User) { n.Edges.Artilces = []*Article{} },
			func(n *User, e *Article) { n.Edges.Artilces = append(n.Edges.Artilces, e) }); err != nil {
			return nil, err
		}
	}
...
func (uq *UserQuery) loadArtilces(ctx context.Context, query *ArticleQuery, nodes []*User, init func(*User), assign func(*User, *Article)) error {
	...
	query.Where(predicate.Article(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.ArtilcesColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.user_artilces
		if fk == nil {
			return fmt.Errorf(`foreign-key "user_artilces" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "user_artilces" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
